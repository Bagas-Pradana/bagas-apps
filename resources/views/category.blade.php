...
<h1 class="px-12 w-[50%] pt-8 text-2xl text-blue-500 font-bold">Postingan Category: {{ $category }}</h1>
{{-- @foreach ($post as $hasil)
<div class="px-12 w-[425px] flex flex-col gap-y-4 pt-4">
        <div class="px-12 w-[425px] flex flex-col gap-y-4 pt-8">
            <h2 class="text-cyan-700 font-bold underline underline-offset-4 decoration-4">
                <a href="/post/{{$hasil->slug }}">{{$hasil->judul }}</a>
            </h2>
            <h2 class="text-xl font-semibold text-slate-900">By: {{$hasil->author->name }}</h2>
            <p class="text-xl text-slate-900">{{$hasil->excerpt }}</p>
        </div>
    </div>
